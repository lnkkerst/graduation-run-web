...
import { ErrorTextStatus } from 'mdui.jq/es/types/JQAjax';
    ...
    methods: {
        async fetchPage(page: number = -1) {
            ...
            try {
                ...
                const where = [] as { kId: string, items: string[] }[];
                for (let i of this.filterItems) {
                    if (i.type !== 1) {
                        continue;
                    }
                    let tmp = { kId: i.abbr, items: [] as string[] };
...
<template>
        ...
        <div class="mdui-collapse-item">
            <div class="mdui-collapse-item-header mdui-valign">
                ...
                <div class="mdui-col-xs-8">
                    <span class="mdui-float-right mdui-m-t-2">数据库中共有 {{ amount }} 条信息</span>
                </div>
            </div>

            <div
                class="mdui-collapse-item-body mdui-m-y-1 mdui-container-fiuld mdui-card mdui-shadow-10 mdui-m-y-2"
            >
                <div class="mdui-p-a-3">
                    <div class="mdui-row mdui-valign mdui-m-b-2">
                        ...
                        <div class="mdui-col-xs-10 mdui-cotainer-fiuld">
                            <div
                                class="mdui-chip mdui-m-l-2 mdui-p-x-1 mdui-ripple"
                                v-for="item in thead"
                                :class="{ 'mdui-color-theme': item.show }"
                    ...
                    <div class="mdui-row mdui-valign mdui-m-y-2" v-for="line in filterItems">
                        <div class="mdui-col-xs-2 mdui-typo-subheading">{{ line.title }}</div>
                        <div class="mdui-col-xs-10 mdui-cotainer-fiuld">
                            <div
                                class="mdui-chip mdui-m-l-2 mdui-p-x-1 mdui-ripple"
        ...
        <table class="mdui-table mdui-table-hoverable" v-if="!tableLoading">
            ...
            <tbody>
                <tr v-for="item of info">
                    ...
                    <td v-show="thead[3].show">{{ item.sex }}</td>
                    <td v-show="thead[4].show">{{ item.campus }}</td>
                    <td v-show="thead[5].show">{{ item.clothingSize }}</td>
                    <td v-show="thead[6].show">{{ item.branch }}</td>
                    <td v-show="thead[7].show">{{ item.phone }}</td>
                    <td v-show="thead[8].show">{{ item.id }}</td>
                    <td v-show="thead[9].show">{{ item.contact }}</td>
                    <td v-show="thead[10].show">{{ item.cPhone }}</td>
                </tr>
            </tbody>
        </table>
    </div>
    <div class="mdui-m-t-3 mdui-m-b-2 mdui-row mdui-valign">
        <div class="mdui-col-xs-8">
            <MduiButton
                mdui-raised
                class="mdui-btn-icon mdui-color-white mdui-shadow-6 mdui-m-l-1"
            ...
            <MduiButton
                mdui-raised
                v-for="i of avaliablePage"
                class="mdui-btn-icon mdui-color-white mdui-shadow-6 mdui-m-l-1"
                @click.prevent="curPage = i"
        ...
        <div class="mdui-col-xs-2">
            <MduiButton
                ...
                @click.prevent="curPage = Math.min(Math.max(1, jumpPage), maxPage)"
            >Go!</MduiButton>
        </div>
    </div>
</template>
